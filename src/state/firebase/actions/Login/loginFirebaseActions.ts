...
import { db } from "../../queuingSystem";
import { UserType } from "../../../data_types/UserType";
import { UserRoleType } from "../../../data_types/UserRoleType";

export const LoginFirebase = async(username: string, password: string) => {
    // users
    const users = [] as UserType[]
    const queryUser = query(collection(db, "users"), where("username", "==", username), where("password", "==", password));
    const userQuerySnapshot = getDocs(queryUser)
