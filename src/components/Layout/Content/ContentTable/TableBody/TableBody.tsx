...
const GenerateTable = (type: string, data: DeviceType[] | ServiceType[] | ProgressionType[] | ReportType[] | UserRoleType[] | UserType[] | UserActivityLogType[], isLite: boolean | undefined) => {
  switch(type)
  {
    ...
    case 'User[]':
      return (
        <Table pagination={{ pageSize: 9, hideOnSinglePage: true, prevIcon: ArrowLeftIcon, nextIcon: ArrowRightIcon, className:'custom-table-pagination' }} dataSource={UserCAD.dataSource(data)} columns={UserCAD.Columns} className='table-body-main-table' />
      )
    case 'UserActivityLog[]':
      return (
        <Table pagination={{ pageSize: 9, hideOnSinglePage: true, prevIcon: ArrowLeftIcon, nextIcon: ArrowRightIcon, className:'custom-table-pagination' }} dataSource={UserActivityLogCAD.dataSource(data)} columns={UserActivityLogCAD.Columns} className='table-body-main-table' />
      )
    default:
      return null
  }
